...
**Default weights** (override via `~/.opencode/loop-config.conf`):
...
At loop start, check for `~/.opencode/loop-config.conf`. If it exists, read the key=value pairs and use them instead of defaults. Format:
...
All results are logged as JSONL to `.opencode/experiments/<YYYY-MM-DD>.jsonl`.
...
1. Create the experiment log directory if it does not exist:
   ```bash
   mkdir -p .opencode/experiments
   ```
2. **Check for existing experiment log** — if `.opencode/experiments/<today>.jsonl` exists, read it to determine the current best metric value and iteration count. Resume from the next iteration number.
...
**Step 1: Review state.** Read the experiment log (`.opencode/experiments/<today>.jsonl`), review git history (`git log --oneline -10`), and identify what has been tried, what worked, and what failed.
...
**Step 7: Log the result.** Append a JSONL entry to `.opencode/experiments/<today>.jsonl`.
