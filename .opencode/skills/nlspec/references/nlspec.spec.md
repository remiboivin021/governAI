...
## Table of Contents
1. [The Nature of the Artifact](#the-nature-of-the-Artifact)
2. [Why NLSpecs Work When They Work](#why-nlspecs-work-when-they-work)
    1. [Behavioral Completeness](#behavioral-completeness)
    2. [Unambiguous Interfaces](#2-unambiguous-interfaces)
    3. [Explicit Defaults and Boundaries](#3-explicit-defaults-and-boundaries)
    4. [Mapping Tables for Translation](#4-mapping-tables-for-translation)
    5. [Testable Acceptance Criteria](#5-testable-acceptance-criteria)
3. [What an NLSpec Is Not](#what-an-nlspec-is-not)
    1. [The Precision-Completeness Distinction](#the-precision-completeness-distinction)
4. [Conceptual Fidelity](#conceptual-fidelity)
5. [Spec Economy](#spec-economy)
6. [Intentional vs. Accidental Ambiguity](#intentional-vs-accidental-ambiguity)
    1. [Intentional Ambiguity](#intentional-ambiguity)
    2. [Accidental Ambiguity](#accidental-ambiguity)
    3. [How to Tell the Difference](#how-to-tell-the-difference)
7. [Appendix A: When Implementing from an NLSpec](#appendix-a-when-implementing-from-an-nlspec)
8. [Appendix B: When Authoring an NLSpec](#appendix-b-when-authoring-an-nlspec)
9. [Appendix C: When Evaluating an NLSpec](#appendix-c-when-evaluating-an-nlspec)
...
If yes, the ambiguity is intentional or at least harmlessthe contradiction reflects actual intent. When you identify a self-contradiction, flag it and request repair. Do not pick a side silently.
...
- **Malformation (self-contradiction)** — the spec asserts two incompatible requirements. Section A says retry three times; Section B says retry five times. This is a document-level defect. Self-contradiction in a specification is never intentional — it is always an error in the document itself. No amount of implementer judgment resolves a contradiction, because any implementation satisfies one requirement by violating the other. Only the spec author can decide which side of 
- **Different implementations making different choices here produce incompatible or incorrect behavior.** If one implementation retries on timeout and another doesn't, callers experience differ
- **Different implementations making different choices here produce incompatible or incorrect behavior.** If one implementation retries on timeout and another doesn't, callers experience different reliability characteristics from the same spec.
- **The spec appears to assume something it never states.** "Process the messages in order" — does "in order" mean insertion order, chronological order, or priority order? The author probably meant one of these but didn't say which.
- **An implementer must guess to proceed.** Intentional ambiguity lets the implementer choose freely. Accidental ambiguity forces the implementer to guess what the author meant. The difference is felt in the implementer's experience: freedom feels like freedom; guessing feels like anxiety.### How to Tell the Difference

The test is: **would two implementations that resolve this ambiguity differently be interchangeable to a caller?**

If yes, the ambiguity is intentional or at least harmlessent reliability characteristics from the same spec.
- **The spec appears to assume something it never states.** "Process the messages in order" — does "in order" mean insertion order, chronological order, or priority order? The author probably meant one of these but didn't say which.
- **An implementer must guess to proceed.** Intentional ambiguity lets the implementer choose freely. Accidental ambiguity forces the implementer to guess what the author meant. The difference is felt in the implementer's experience: freedom feels like freedom; guessing feels like anxiety.
